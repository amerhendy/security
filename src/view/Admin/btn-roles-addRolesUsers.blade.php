<a href="javascript:void(0)" onclick="addRolesUsers(this)" data-entry="{{$entry->getKey()}}" data-mdb-ripple-duration="0s" class="btn btn-sm btn-success" data-button-type="addRolesUsers"><i class="fa fa-users"></i></a>
@push('after_scripts') @if (request()->ajax()) @endpush @endif
@loadOnce('addRolesUsers')
<script>
    if (typeof addRolesUsers != 'function') {
        $("[data-button-type=addRolesUsers]").unbind('click');
        function addRolesUsers(button){
            var RoleId=$(button).data('entry')
            var type='all';
            var ModalId='addRolesUsers';
            var ResultFn='addUsersToRole';
            createModal(button,ModalId,"{{trans('SECLANG::permissionmanager.users')}}");
            sendSecRequestToGetAll('all',RoleId,"{{url('Security/role/fetch/Users')}}",{type,RoleId},ModalId,ResultFn,'name','email');
            //getallPerms('all',RoleId,'addRolesUsers');
            //Security/Roles/fetch/Perms
        }
    }
    function addUsersToRole(RoleId,id,action){
            if(action == 'minus'){
                action='plus';
            }else{
                action='minus';
            ...
            jQuery.ajax({
                url:"{{url('Security/role/fetch/AddUsers')}}",
                data:{RoleId,id,action},
            }).done(function(data){
                $('i[data-int='+id+']').attr('class','fa fa-'+data[0]);
                $(div).attr('onclick',"addUsersToRole("+RoleId+","+id+",'"+data[0]+"')");
            });
        }
</script>
@endLoadOnce
