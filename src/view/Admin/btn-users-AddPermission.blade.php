<a href="javascript:void(0)" onclick="addUserPerms(this)" data-entry="{{$entry->getKey()}}" data-mdb-ripple-duration="0s" class="btn btn-sm btn-success" data-button-type="addUserPerms"><i class="fa fa-lock"></i></a>
@push('after_scripts') @if (request()->ajax()) @endpush @endif
@loadOnce('addUserPerms')
<script>
    if (typeof addUserPerms != 'function') {
        $("[data-button-type=addUserPerms]").unbind('click');
        function addUserPerms(button){
            var UserId=$(button).data('entry')
            var type='all';
            var ModalId='addPemissionsToUser';
            var ResultFn='addPemissionsToUser';
            createModal(button,ModalId,"{{trans('SECLANG::permissionmanager.permission_plural')}}");
            sendSecRequestToGetAll('all',UserId,"{{url('Security/user/fetch/Perms')}}",{type,UserId},ModalId,ResultFn,'name');
        }
        function addPemissionsToUser(UserId,id,action){
            if(action == 'minus'){
                action='plus';
            }else{
                action='minus';
            ...
            jQuery.ajax({
                url:"{{url('Security/user/fetch/AddPers')}}",
                data:{UserId,id,action},
            }).done(function(data){
                $('i[data-int='+id+']').attr('class','fa fa-'+data[0]);
                $(div).attr('onclick',"addPemissionsToUser("+UserId+","+id+",'"+data[0]+"')");
            });
        }
    }
</script>
...
function sendSecRequestToGetAll(Type,RoleId,URL,DATA,DivId,ResultFn,fstText,ndText=null){
    ...
    jQuery.ajax({
        ...
        if(isObjext(selectedDataIds)){
            selectedDataIds = Object.values(selectedDataIds);
        }
