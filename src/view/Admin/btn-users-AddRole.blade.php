<a href="javascript:void(0)" onclick="addRolesToUserS(this)" data-entry="{{$entry->getKey()}}" data-mdb-ripple-duration="0s" class="btn btn-sm btn-success" data-button-type="addRolesToUserS"><i class="fa-brands fa-critical-role"></i></a>
@push('after_scripts') @if (request()->ajax()) @endpush @endif
@loadOnce('addRolesToUserS')
<script>
    if (typeof addRolesToUserS != 'function') {
        $("[data-button-type=addteamUser]").unbind('click');
        function addRolesToUserS(button){
            var UserId=$(button).data('entry')
            var type='all';
            var ModalId='addRolesToUser';
            var ResultFn='addRolesToUser';
            createModal(button,ModalId,"{{trans('SECLANG::permissionmanager.roles')}}");
            sendSecRequestToGetAll('all',UserId,"{{url('Security/user/fetch/Roles')}}",{type,UserId},ModalId,ResultFn,'name');
        }
        function addRolesToUser(UserId,id,action){
            if(action == 'minus'){
                action='plus';
            }else{
                action='minus';
            ...
            jQuery.ajax({
                url:"{{url('Security/user/fetch/AddRoles')}}",
                data:{UserId,id,action},
            }).done(function(data){
                $('i[data-int='+id+']').attr('class','fa fa-'+data[0]);
                $(div).attr('onclick',"addRolesToUser("+UserId+","+id+",'"+data[0]+"')");
            });
        }
    }
</script>
